...
\author[1]{Miguel Angel Jim\'{e}nez-Urias}
...
themselves; \ac{DMR++} inventories expose the cloud retrieval structure \citep{gallagher_2024_14025927}; and
...
checks of metadata attributes at the variable level (\textit{e.g.} units, coordinates) and
...
with our \ac{DAAC} collaborator. The subsections below describe how the project
first makes those collections crawlable by generic \ac{AI} tools, then produces
validated collection-level \texttt{usage.md} metadata documents. We originally
intended to make these documents seamlessly available through the existing
\ac{OPeNDAP} service responses, but given time constraints of the project's
funding,\footnote{EED3 contract funding ends on 29 January 2027.} we will leave that
integration as part of moving this work into operations. The generated
\texttt{usage.md} documents will be available to external users through a
separate S3 bucket.
...
crawl broader coverage when feasible, but the commitment of the project is to
...
some collections, not all of this information will be relevant.}
...
Safety and security are important concerns for this system and to guard against
common security threats, the system will use a tool-based architecture with 
strict input/output schemas, and manual tool invocation~\cite{maloyan2026prompt} using, e.g., LangChain.
This will provide a way to ensure that Web APIs surfaced by NASA systems (e.g.,
\ac{CMR}) only receive sanitized inputs~\cite{liu2023prompt}\cite{zhan-etal-2024-injecagent}.
...
The generated \texttt{usage.md} documents will be stored in an S3 bucket managed
by \ac{OPeNDAP} and separate from any of the \acs{DAAC} artifacts. They will be
available to external users through a separate S3 bucket, and they will be made
available through the \ac{OPeNDAP}/Hyrax service responses when the work is made
operational.
...
During this project, we will produce three new concrete outputs:
...
\item \textbf{A collection of usage.md documents.} We will deliver
a set of \texttt{usage.md} documents for selected collections in a separate S3
in the \ac{EDC} bucket managed by \ac{OPeNDAP}. The visibility of these
documents will be controlled by \ac{OPeNDAP} based on guidance by NASA \ac{ESDS}.
...
be measured by the fraction of selected collections and granules that can be accessed.
...
Deployment to operations is beyond the scope of this project. However, the path
to operations is to embed the content of the \texttt{usage.md} 
documents into the standard responses provided by the \ac{OPeNDAP}/Hyrax service. This will enable 
autonomous clients to use the embedded guidance to make correct data access and
subsetting requests.

\section{Timeline}
This project is designed to be completed in 6 months and has two main parts.
The work will start on August 3, 2026, and end on January 29, 2027, aligned 
with the EED3 contract funding period.

\subsection*{Part 1: AI-discoverable and crawlable service interfaces} 
\begin{description}
    \item [Month 1] Deploy and test the candidate Hyrax virtual directory interface
 and define target validation collections. 
\end{description}
...
\begin{description}
    ...
    collections. 
    \item [Month 5] Run expert review and revise validators based on the review. 
\end{description}
...
\begin{description}
    ...
    \item [Month 6] Package the open-source artifacts, final report, and presentation/demo, and
...
Gallagher} will lead the project and provide guidance on the Hyrax/\acs{OPeNDAP} architecture, Virtual Directory
interface, contribute to agentic \ac{AI} development and
software engineering. {\em Dan Holloway} will contribute agentic \acs{AI}
development and software engineering. Both Holloway and Gallagher have prior
experience building \ac{AI} systems. {\em Miguel Angel Jim\'{e}nez-Urias} will
collaborate on agentic \acs{AI} development, use of \ac{PyDAP}domain review, and testing as an
\acs{OPeNDAP}-funded contributor. Miguel has a background in machine learning
and physical oceanography.

{\em Ed Armstrong} at NASA
\ac{JPL} \acs{PODAAC} will support domain validation and testing, help select
...
\textbf{Indirect costs} For \ac{OPeNDAP}, there are no indirect costs added to the
budget since payment is through the existing \ac{ESDIS}
EED3\footnote{NASA Earth Observing System Data and Information System Evolution and
Development-3} contract. Ed Armstrong's cost does include indirect
costs. See Appendix~\ref{app-jpl-budget} for details.\\
...
rates for the project team members, as detailed in the budget table in Appendix~\ref{app-budget}. The
total personnel cost is \$173,177. {\em James Gallagher} and {\em Dan Holloway}
will be funded by the NASA EED3 contract. {\em Ed
Armstrong} will be funded by the \ac{PODAAC}.
{\em Miguel\`{A}ngel Jim\`{e}nez} will be funded by \ac{OPeNDAP} as part
...
\hline
Miguel Angel Jim\'{e}nez-Urias & 2 months\footnote{Funded by \ac{OPeNDAP}} & Agentic \ac{AI} development, domain expert and testing; collaborator at \acs{OPeNDAP} \\
\hline
